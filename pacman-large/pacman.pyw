...
class path_finder():
    ...
    def ResizeMap(self, numRowsCols):
    ...
    def AddToClosedList(self, rowCol):
        row, col = rowCol
        self.closedList.append((row, col))

    def IsInClosedList(self, rowCol):
        row, col = rowCol
    ...
    def SetParent(self, rowCol, parentRowCol):
        parentRow, parentCol = parentRowCol
        row, col = rowCol
        self.map[self.Unfold((row, col))].parent = (parentRow, parentCol)

    def GetParent(self, rowCol):
        row, col = rowCol
...
class level():
    ...
    def SetMapTile(self, rowCol, newValue):
        row, col = rowCol
        self.map[(row * self.lvlWidth) + col] = newValue

    def GetMapTile(self, rowCol):
        row, col = rowCol
    ...
    def CheckIfHitWall(self, possiblePlayerX_Y, rowCol):
        possiblePlayerX, possiblePlayerY = possiblePlayerX_Y
        row, col = rowCol
        numCollisions = 0
    ...
    def CheckIfHit(self, playerX_Y, x_y, cushion):
        playerX, playerY = playerX_Y
        x, y = x_y
    ...
    def CheckIfHitSomething(self, playerX_Y, rowCol):
        playerX, playerY = playerX_Y
        row, col = rowCol
        for iRow in range(row - 1, row + 2, 1):
            for iCol in range(col - 1, col + 2, 1):
